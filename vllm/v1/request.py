...
class Request:
    ...
    def trace_end_queuing(self) -> None:
        """Transition request out of the queue. Closes vllm.request.queue span."""
        if self.arrival_time_ns is None:
            return

        now = time.time_ns()
        self.emit_span("vllm.request.queue", self.arrival_time_ns, now)
        self.arrival_time_ns = None  # prevent double queue spans

    def trace_end_kv_transfer(self, aborted: bool = False) -> None:
        """Transition request out of KV transfer. Closes
        vllm.request.wait_remote_kv span.
        """
    ...
    def trace_end_prefill(self, aborted: bool = False) -> None:
        ...
        self.emit_span(
            "vllm.request.prefill", self.prefill_start_time_ns, now, attributes
        )
        self.prefill_start_time_ns = None

    def trace_end_decode(self, aborted: bool = False) -> None:
        """Transition request out of decode. Closes vllm.request.decode span."""
        ...
        attributes = {"aborted": True} if aborted else None
        self.emit_span(
            "vllm.request.decode", self.decode_start_time_ns, now, attributes
        )
