...
class Request:
    ...
    def emit_span(
        ...
        """Helper to manually emit an OTel trace span linked to this request's context."""
        from vllm.tracing import extract_trace_context, instrument_manual

        trace_context = (
            extract_trace_context(self.trace_headers)
    ...
    def trace_end_kv_transfer(self, aborted: bool = False) -> None:
        ...
        self.emit_span("vllm_async_kv_transfer", self.async_kv_load_start_time_ns, now, attributes)
    ...
    def trace_cleanup(self) -> None:
        """Catch-all transition to clean up any lingering active spans when request is freed."""
        is_aborted = self.status == RequestStatus.FINISHED_ABORTED or self.status == RequestStatus.FINISHED_ERROR
